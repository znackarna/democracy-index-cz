...
- **Iterace 6:** Next.js + Tailwind dashboard, static export, deploy na Vercel. Public dispute link u každé události.
- **Iterace 7:** Self-audit infrastruktura — oddělený auditor Claude pass, daily reports v `data/reports/`, anomaly detection (auto GitHub issue), source-count → severity cap rule.
- **Iterace 8:** GitHub Actions (`weekly-pipeline` s auto-merge po self-audit, `recompute-scores`, `monthly-spotcheck`, `dispute-handler`).
- **Iterace 9+:** Backtesting na 2018–2020, kvartální validace proti V-Dem/EIU.
